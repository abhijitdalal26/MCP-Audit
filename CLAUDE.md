...
| CLI | Go (offline by default — zero data sent) |
| Docker | Multi-stage image bundling CLI + API for self-hosting |
...
go test ./...
./bin/mcpaudit.exe scan <config.json>                         # fully offline
./bin/mcpaudit.exe scan <config.json> --api-url http://localhost:8000  # remote API

# Docker (self-hosting — API + web UI, CLI runs offline natively)
docker build -t mcpaudit .
docker run -p 8000:8000 mcpaudit                              # API at localhost:8000
...
## Build Order
1. **Stage 1 (MVP — current)**: Web paste-and-scan — 54 checks, SARIF, AI-BOM
2. **Stage 2**: Docker image (self-hosting) + GitHub Action + tooltrust subprocess wrapper
3. **Stage 3**: LLM triage (Claude API) — semantic analysis of tool descriptions
4. **Stage 4**: eBPF dynamic sandbox + SIEM integration + scheduled scans + Slack alerts
...
- Docker image size: Python + Go multi-stage build targets ~150MB compressed
